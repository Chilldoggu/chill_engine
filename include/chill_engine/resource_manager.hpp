...
public:
	...
	auto load_model(const std::wstring& a_dir, bool a_flip_UVs = false, bool a_gamma_corr = false) -> Model;
	auto create_model(const std::vector<Mesh>& a_meshes) -> Model;

	auto load_texture(const std::wstring& a_path, TextureType a_type, int a_unit_id, bool a_flip_image, bool a_gamma_corr) -> Texture;
	auto load_cubemap(const std::vector<std::wstring>& a_paths, int a_unit_id, bool a_flip_images, bool a_gamma_corr) -> Texture;
